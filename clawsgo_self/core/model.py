...
读取本地配置 `~/.clawsgo-self/env`（JSON/KEY=VAL），提供 OpenAI 兼容的
chat.completions 调用。为规避硬依赖，用标准库 urllib 实现；若安装了 openai
包可用其实现（可选）。

配置读取优先级：环境变量 > 本地配置文件。
配置文件示例（~/.clawsgo-self/env）：
...
_HOME_ENV = Path.home() / ".clawsgo-self" / "env"
...
def chat(
    ...
    *,
    ...
    if not configured():
        raise RuntimeError(
            "未配置 LLM：请在 ~/.clawsgo-self/env 设置 CLAWSGO_SELF_LLM_BASE(=端点) "
